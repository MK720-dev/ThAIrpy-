...
def draw_prediction_on_image(
  ...
  fig.canvas.draw()
  # Convert the figure to a PNG image using the canvas
  # Create a BytesIO object to store the image data
  image_data = io.BytesIO()
  fig.canvas.print_png(image_data)  # Pass the BytesIO object to print_png
  image_data.seek(0)  # Reset the file pointer to the beginning
  png_data = image_data.read() # Read the image data from the BytesIO object

  # Convert the PNG data to an image using PIL
  img = Image.open(io.BytesIO(png_data))
  img.show()
  image_from_plot = np.array(img) # Assign image_from_plot here using img
...
# Set input size based on model selection
MODEL_INPUT_SIZES = {
...
if "tflite" in model_name:
    ...
    print(f"Model {model_name} loaded successfully with input size {input_size}.")
